...
import { getProviderAccountOverview } from '../data/mockPhase6';
import {
  Upload,
  Camera,
  MapPin,
...
function ProviderAccountOverview() {
  ...
  // [MOCK] Currently fed by getProviderAccountOverview() from mockPhase6.
  const [providerStats, setProviderStats] = useState({ completed: 0, active: 0 });
  useEffect(() => {
    getProviderAccountOverview().then(o => setProviderStats({ completed: o.completedJobs, active: o.activeBookings }));
  }, []);
    ...
    <div className="min-h-screen bg-white dark:bg-[#0f1117] font-sans">
      ...
      <div className="flex items-center justify-center p-5 pb-20 md:pb-5">
        ...
        >
          ...
          <button 
            onClick={upload.openModal}
            className="bg-gradient-to-r from-sky-600 to-sky-800 text-white px-6 py-3 rounded-xl font-semibold cursor-pointer transition-all duration-300 flex items-center justify-center gap-2 mx-auto mb-5 shadow-md hover:-translate-y-0.5 hover:shadow-xl"
          >
