...
const ServiceCard = memo(({ service }) => {
  ...
  return (
    <Link to={`/lucid/services/${service.slug}`} className="block h-full">
      <div className="h-full bg-white dark:bg-[#1a1f2e] border border-gray-200 dark:border-[#1e293b] rounded-2xl p-6 flex flex-col items-center justify-center gap-3 hover:border-sky-500 hover:shadow-md transition-all duration-200 cursor-pointer group">
        <div className="w-12 h-12 rounded-xl bg-sky-50 dark:bg-primary/10 flex items-center justify-center group-hover:bg-sky-100 dark:group-hover:bg-primary/20 transition-colors">
    ...
    <div className="w-full min-h-screen bg-gray-50 dark:bg-[#0f1117]">
      ...
      <header className="relative z-10" style={{ minHeight: 460 }}>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-5 py-20" style={{ minHeight: 460 }}>
          ...
          <p className="text-sky-200 text-base md:text-lg mb-8">
            Find trusted professionals for any job across Ghana
          </p>
          <form
            onSubmit={handleSearch}
            className="max-w-2xl w-full mx-auto flex bg-white dark:bg-[#252b3b] border border-gray-300 dark:border-[#2d3748] rounded-xl shadow-md focus-within:ring-2 focus-within:ring-sky-500 transition-shadow"
          >
            <SearchAutocomplete
              value={query}
              onChange={setQuery}
      ...
      <section className="max-w-6xl mx-auto px-5 py-14">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-slate-100">
            Popular services <span className="text-sky-700">near you</span>
          </h2>
          <Link
            to="/lucid/services/all"
            className="text-sky-700 dark:text-blue-400 hover:text-sky-800 dark:hover:text-blue-300 font-medium text-sm flex items-center gap-1 group"
          >
      ...
      {/* ── All Categories CTA ── */}
      <section className="bg-sky-700 py-14 px-5 text-center">
        <h2 className="text-white text-2xl md:text-3xl font-bold mb-3">
          Browse all service categories
        </h2>
        <p className="text-sky-200 mb-7">10 categories · 50+ services across Ghana</p>
        <Link
          to="/lucid/services/all"
          className="inline-block bg-white text-sky-700 font-bold px-8 py-3 rounded-full hover:bg-sky-50 transition-colors shadow"
        >
          See all categories
        </Link>
      </section>
